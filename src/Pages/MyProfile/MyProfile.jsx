...
const MyProfile = () => {
  ...
  // 2. If role is member, fetch agreement
  const { data: agreement, isLoading: agreementLoading } = useQuery({
    queryKey: ['agreement', user?.email],
    enabled: userRole === 'member' && !!user?.email,
    queryFn: async () => {
  ...
  const isLoading = roleLoading || (userRole === 'member' && agreementLoading);
              ...
              <tbody>
                <tr className="hover:bg-gray-50 dark:hover:bg-gray-600">
                  <td className="py-2 px-4 border-b">
                    {userRole === 'member' ? new Date(agreement?.acceptedDate).toLocaleDateString() : 'None'}
                  </td>
                  <td className="py-2 px-4 border-b">
                    {userRole === 'member' ? agreement?.floorNo : 'None'}
                  </td>
                  <td className="py-2 px-4 border-b">
                    {userRole === 'member' ? agreement?.blockName : 'None'}
                  </td>
                  <td className="py-2 px-4 border-b">
                    {userRole === 'member' ? agreement?.apartmentNo : 'None'}
                  </td>
                </tr>
              </tbody>
            </table>
